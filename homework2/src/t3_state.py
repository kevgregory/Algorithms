...
class T3State:
    ...
    def get_transitions(self) -> Iterator[tuple["T3Action", "T3State"]]:
        ...
        for tile in open_tiles:
            ...
            for move in valid_moves:
                ...
                resultant_state = self.get_next_state(action)
                transitions.append((action, resultant_state))

        transitions.sort(key=lambda x: (x[0].col(), x[0].row(), x[0].move()))
    ...
    def is_terminal(self) -> bool:
        """
    Check if the game is finish, either win lose or tie.
    
    Returns:
    - bool: True if game's over (win or tie). False if there's still some fight left.
    """
